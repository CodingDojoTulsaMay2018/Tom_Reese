...
class UserManager(models.Manager):
    ...
    def login_validator(self, postData):
        errors = {}        
        print("Validating a returning cigar guy")
        ...
        if user:
            ...
            if bcrypt.checkpw(postData['password'].encode(), user.password.encode()):
                print("password match, cigar guy logged in successfully")
                return errors
            else:
    ...
    def admin_validator(self, postData):
        errors = {}        
        print("Validating an admin cigar guy")
        user = User.objects.filter(email=postData['email'])
        print(user)
        if user:
            user = User.objects.get(id=user[0].id)
            print(user,"is the user")
            if user.admin == (postData['level']):
                print("data match, cigar guy admin logged in successfully")
                return errors
            else:
                print("failed password")
                errors['level'] = "Admin not found"
            return errors
        else:
            print("cigar admin not found")
            errors['email'] = "Account not found"
            return errors
...
class User(models.Model):
    ...
    def __repr__(self):
        return "<User: {} |{} {} {} {} {} {} {}>".format(self.id, self.first_name, self.last_name, self.email, self.desc, self.admin, self.password, self.created_at)
    objects = UserManager()
